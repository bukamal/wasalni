(function() {
  ...
  function initMap() {
    ...
    } else setPickupLocation(defaultLat, defaultLng);
  ...
  function showNearbyDriversOnMap() {
    map.eachLayer(layer => {
      if (layer instanceof L.Marker && layer !== pickupMarker && layer !== dropoffMarker && layer !== driverMarker) map.removeLayer(layer);
    });
    allDrivers.forEach(driver => {
      if (driver.lat && driver.lng) {
        ...
        }).addTo(map).bindPopup(`${driver.full_name || 'سائق'} (${driver.distance} كم)`);
      }
    });
  }
  ...
  function showNearbyDriversList() {
    ...
    allDrivers.forEach(driver => {
      ...
      div.innerHTML = `<div class="info"><strong>${driver.full_name || 'سائق'}</strong><small>🚘 ${driver.car_model || 'سيارة'} · ${driver.distance} كم</small></div><span class="badge badge-accepted">متاح</span>`;
  ...
  async function trackDriver() {
    ...
    try {
      ...
      if (driver && driver.current_lat && driver.current_lng) {
        const lat = driver.current_lat, lng = driver.current_lng;
  ...
  function startTracking(ride) {
    activeRide = ride;
    document.getElementById('rideStatus').innerHTML = '<div class="status-header"><h3>🚗 السائق في الطريق</h3></div><p class="status-text">يمكنك تتبع السائق على الخريطة</p>';
